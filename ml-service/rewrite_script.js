...
const startMarker = 'function showInPageWarning(type, data) {';
const endMarkerStr = '    document.getElementById("phishguard-continue").onclick = () => {\r\n      overlay.remove();\r\n      reportToBackground("userAction", {\r\n        action: "continued_anyway",\r\n        alertType: type,\r\n      });\r\n    };\r\n  }';
const endMarkerStrAlt = '    document.getElementById("phishguard-continue").onclick = () => {\n      overlay.remove();\n      reportToBackground("userAction", {\n        action: "continued_anyway",\n        alertType: type,\n      });\n    };\n  }';


const idxStart = content.indexOf(startMarker);
let idxEnd = content.indexOf(endMarkerStr);
if (idxEnd === -1) {
    idxEnd = content.indexOf(endMarkerStrAlt);
}

if (idxStart !== -1 && idxEnd !== -1) {
    const startStr = content.substring(0, idxStart);
    const endStr = content.substring(idxEnd + (idxEnd === content.indexOf(endMarkerStr) ? endMarkerStr.length : endMarkerStrAlt.length));
    
    const newFunc = `function showInPageWarning(type, data) {
    ...
    overlay.innerHTML = \`
        <style>
            ...
            .pg-badge {
                ...
                box-shadow: 0 0 10px \${themeColorGlow};
            }
            ...
            .pg-btn-primary {
                ...
                color: #fff;
                border: none;
                box-shadow: 0 4px 15px \${themeColorGlow};
            }
            .pg-btn-primary:hover {
                background: #fff;
                color: \${themeColor};
                transform: translateY(-2px);
                box-shadow: 0 8px 25px \${themeColorGlow};
            }
        ...
        <div id="pg-sec-box">
            ...
            <div class="pg-content">
                ...
                <div class="pg-panel pg-recommendation">
                    <p><strong>Action Required</strong> \${warningInfo.recommendation}</p>
                </div>
            </div>

            <div class="pg-actions">
                <button id="phishguard-go-back" class="pg-btn pg-btn-primary">
                ...
                <button id="phishguard-continue" class="pg-btn pg-btn-secondary">
                    Bypass Warning
                </button>
            </div>

            <div class="pg-footer">
                <span></span> PhishGuard Neural Engine Active
            </div>
        </div>
    \`;
    ...
    fs.writeFileSync(filePath, startStr + newFunc + endStr);
